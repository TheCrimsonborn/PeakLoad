...
## 2026-05-19 - [Live Collections and getElementsByClassName]
**Learning:** In virtually all browser engines, `getElementsByClassName` is significantly faster than `querySelectorAll` because it returns a live `HTMLCollection` and is highly optimized. `querySelectorAll` has to parse a CSS selector and generate a static `NodeList`. Also, live collections are beneficial when you want dynamically added elements to be automatically included in subsequent queries. Replacing a live collection with a static one can introduce regressions if newly added elements need to be processed.
**Action:** Do not prematurely optimize DOM queries by replacing `getElementsByClassName` with `querySelectorAll`, as it acts as an anti-optimization and risks functional regressions.
